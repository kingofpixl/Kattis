...
void mst(int n, int m, Edge elist[], int index[], int& size,int diffState)
{
  UnionFind uf(n);
  int states = n;
  ...
  cout << fixed << setprecision(0) << "Case #" << cases++ << ": " << states << " " << roads << " " << railroads << endl;
}
//done and accepted
int main(void)
{
   ...
   mst(n, k, elist, index, t, diffState);
    }
   return 0;
}