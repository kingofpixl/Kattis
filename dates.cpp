...
int main(){
    int T, YEAR, MONTH, DAY, parse,addDays,setnum;
    ...
    while (T--)
    {
        ...
        DAY = stoi(input.substr(parse+1));
        Date date(YEAR,MONTH,DAY);
        cin >> addDays;
        date.addDay(addDays);
        setnum++;

        cout << "Case " << setnum << ": " << date.yyyy << "-";
        cout << MONTHS[date.mm];
        if (date.dd < 10) //if the day is less than 10 we have to add a 0 infront
        {
            cout << "-0" << date.dd << endl;
        }
        else
